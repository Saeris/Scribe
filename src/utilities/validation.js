...
export const validateViewerId = (context, id) => {
  ...
  const results = isNullish(viewerType) ? [ ] : isNullish(id)
    ? [ `Must pass a viewerId to ${func}.` ] : !isGlobalId(id)
    ? [ `viewerId passed to ${func} is invalid.` ] : typeFromGlobalId(id) !== viewerType
